...
const apiUrl =
    'http://api.steampowered.com/ISteamNews/GetNewsForApp/v0002/?appid=620980&count=1&maxlength=0&format=json'
...
export default {
                    ...
                    for (let line of contents.split('\n')) {
                        if (line.match(/STEAM_CLAN_(LOC_)?IMAGE/)) {
                            if (!image) {
                                if (
                                    line.match(
                                        /\[img\]\{STEAM_CLAN_(LOC_)?IMAGE\}.+\[\/img\]/
                                    )
                                ) {
                                    image = line.replace(
                                        /\[img\]\{STEAM_CLAN_(?:LOC_)?IMAGE\}(.+)\[\/img\]/,
                                    ...
                                    line.match(
                                        /\[img src=\"{STEAM_CLAN_(LOC_)?IMAGE\}[^\"]+\"\]\[\/img\]/
                                    )
                                ) {
                                    image = line.replace(
                                        /\[img src=\"{STEAM_CLAN_(?:LOC_)?IMAGE\}([^\"]+)\"\]\[\/img\]/,
                    ...
                    if (lastNews.title) {
                        ...
                        if (image) {
                            try {
                                ...
                                if (!imageRequest.ok)
                                    throw new Error(
                                        "Récupération de l'image impossible"
                                    )
                                const imageData =
                                    await imageRequest.arrayBuffer()
                        ...
                        if (!newUpdate) {
                            Logger.log(
                                'BSUpdate',
                                'INFO',
                                'Nouvelle mise à jour de Beat Saber'
                            )
                            const BSUpdate =
                                await BSUpdateModel.create(updateData)
                            return BSUpdate
                        }
                    }
                }
    ...
    async postUpdate(client: Client, update: BSUpdateModel) {
        ...
        const updateChannel = guild.channels.cache.get(
            config.guild.channels['bs-updates']
        ) as TextChannel

        if (update.title && update.content) {
            await updateChannel.send({
                content: roleMention(config.guild.roles['Beat Saber Update'])
            })
            ...
            for (let i = 0; i < content.length; i++) {
                ...
                if (
                    message.join('\n').length + content[i].length >= 1500 ||
                    i === content.length - 1
                ) {
                    const m = await updateChannel.send({
