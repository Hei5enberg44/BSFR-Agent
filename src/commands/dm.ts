...
export default {
        ...
        .addUserOption((option) =>
            option
                ...
                .setDescriptionLocalization(
                    'fr',
                    'Membre à qui envoyer un message privé'
                )
                .setRequired(true)
        )
    ...
    allowedChannels: [config.guild.channels['agent-dm']],
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        try {
            ...
            const getThreadChannel = async (
                channel: TextChannel,
                threadId: string
            ) => {
                const thread = await new Promise<ThreadChannel | null>(
                    (resolve, reject) => {
                        channel.threads
                            .fetch(threadId)
                            .then((thread) => {
            ...
            if (!createdThread) {
                ...
                if (!thread.id) {
                    Logger.log(
                        'DMCommand',
                        'ERROR',
                        `Échec de la création du thread ${member.username}`
                    )
                    throw new CommandInteractionError(
                        Locales.get(interaction.locale, 'thread_creation_error')
                    )
                } else {
                    Logger.log(
                        'DMCommand',
                        'INFO',
                        `Thread "${member.username}" créé`
                    )
                ...
                if (!thread) {
                    Logger.log(
                        'DMCommand',
                        'ERROR',
                        `Thread "${member.username}" introuvable`
                    )
                    throw new CommandInteractionError(
                        Locales.get(
                            interaction.locale,
            ...
            if (thread) {
                ...
                await thread.send({
                    content: `${userMention(interaction.user.id)}: ${message}`
                })
                ...
                try {
                    ...
                    Logger.log(
                        'DMCommand',
                        'INFO',
                        `Message privé envoyé à ${member.username}`
                    )
                    ...
                    Logger.log(
                        'DMCommand',
                        'ERROR',
                        `Le message privé à ${member.username} n'a pas pu être envoyé`
                    )
                    await interaction.reply({
                        content: Locales.get(
                            interaction.locale,
                ...
                throw new CommandInteractionError(
                    Locales.get(interaction.locale, 'dm_error')
                )
            }
        } catch (error) {
