import { Guild, SlashCommandBuilder, PermissionFlagsBits, ChatInputCommandInteraction, GuildMemberRoleManager, roleMention } from 'discord.js'
        ...
        try {
            ...
            switch(action) {
                ...
                case 'add': {
                    ...
                    const roleGroup = <RoleListItem>_roles.find(g => g.id === roleGroupId)
                    const roleGrouproleList = roleGroup.roles

                    const roleToAdd = <RoleItem>roleGrouproleList.find(r => r.name === interaction.options.getString('role', true))

                    embed.setTitle(Locales.get(interaction.locale, 'role_add'))

                    const role = guild.roles.cache.find(r => r.name === roleToAdd.name)
                    if(role) {
                ...
                case 'remove': {
                    ...
                    const roleToRemove = <RoleItem>roleGrouproleList.find(r => r.name === interaction.options.getString('role', true))

                    embed.setTitle(Locales.get(interaction.locale, 'role_remove'))
                    
                    const role = guild.roles.cache.find(r => r.name === roleToRemove.name)
                    if(role) {
                        ...
                        embed.setColor('#E74C3C').setDescription(Locales.get(interaction.locale, 'role_does_not_exist_error', roleToRemove.name))
                    }
