import { SlashCommandBuilder, PermissionFlagsBits, ChatInputCommandInteraction, ActionRowBuilder, ButtonBuilder, ButtonStyle, CollectorFilter, ComponentType, ButtonInteraction, TextChannel } from 'discord.js'
            ...
            switch(action) {
                case 'add': {
                    ...
                    if(cities.length > 0) {
                        ...
                        const getComponents = (page: number = 0): ActionRowBuilder<ButtonBuilder>[] => {
                            const components = []
                            ...
                            for(let row = 0; row < Math.ceil(_cities.length / 5); row++) {
                                const citiesButtons = new ActionRowBuilder<ButtonBuilder>()
                        ...
                        const filter: CollectorFilter<[ButtonInteraction]> = i => i.customId.includes(customId) && i.user.id === interaction.user.id

                        const collector = (<TextChannel>interaction.channel).createMessageComponentCollector<ComponentType.Button>({ filter, time: 15000 })
