        ...
        try {
            ...
            if (!thread)
                throw new CommandInteractionError(
                    Locales.get(interaction.locale, 'dm_wrong_channel_error')
                )
            ...
            if (!dmEnabled)
                throw new CommandInteractionError('Les DM sont désactivés')
            ...
            if (member) {
                try {
                    ...
                    Logger.log(
                        'RCommand',
                        'INFO',
                        `Message privé envoyé à ${member.user.username}`
                    )
                ...
                Logger.log(
                    'RCommand',
                    'ERROR',
                    `Impossible de récupérer le membre "${thread.memberId}"`
                )
                await interaction.reply({
                    content: Locales.get(
                        interaction.locale,
